        ...
        for k, v in kwargs.items():
            if k in self.expected_inputs:
                ...
                if not v.dtype == expected_dtype:
                    raise TypeError(f"Expected dtype {expected_dtype}, got {v.dtype} for input: {k}")

                expected_shape = self.expected_inputs[k]["shape"]
                # Relaxed check for flexible shapes
                if len(v.shape) != len(expected_shape):
                    raise TypeError(f"Expected rank {len(expected_shape)}, got {len(v.shape)} for input: {k}")
...
def get_resource_type(resources_dir: str) -> str:
    ...
    if len(set(extensions)) == 1:
        ...
        raise ValueError(
            f"Multiple file extensions found at {resources_dir}.Cannot infer resource type from contents."
        )
...
def _load_mlpackage(submodule_name, mlpackages_dir, model_version, compute_unit, sources=None):
    """
    ...
    if sources == "packages":
        ...
        fname = f"Stable_Diffusion_version_{model_version}_{submodule_name}.mlpackage".replace("/", "_")
        mlpackage_path = os.path.join(mlpackages_dir, fname)

        if not os.path.exists(mlpackage_path):
            raise FileNotFoundError(f"{submodule_name} CoreML model doesn't exist at {mlpackage_path}")
    ...
    return CoreMLModel(mlpackage_path, compute_unit, sources=sources, optimization_hints=optimization_hints)
...
def _load_mlpackage_controlnet(mlpackages_dir, model_version, compute_unit):
    ...
    if not os.path.exists(mlpackage_path):
        raise FileNotFoundError(f"controlnet_{model_name} CoreML model doesn't exist at {mlpackage_path}")
